...
/**
 * \file    test/functional/DolibarrAssistantFunctionalTest.php
...
/**
 * Class DolibarrAssistantFunctionalTest
 *
 ...
 *
 * @package Testdolibarrassistant
 */
class DolibarrAssistantFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
...
//        )
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modDolibarrAssistant')->click();
		} else {
			// Disable the module
			$this->byHref('modDolibarrAssistant')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/dolibarrassistant/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('dolibarrassistant/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/dolibarrassistant/admin/about.php');
		$this->authenticate();
		return $this->assertContains('dolibarrassistant/admin/about.php', $this->url(), 'About page');
	}
...
return $this->assertEquals(
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('dolibarrassistantwidget1', $this->source(), "Box enabled");
	}
...
return $this->assertContains(
			'interface_99_modDolibarrAssistant_DolibarrAssistantTriggers.class.php',
...
return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modDolibarrAssistant_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
